...
/**
  ...
  用于监控投币机的引脚电平变化
  P0_4用于获取投币器信号
  P0_6用于获取彩票机出了一个彩票信号，不需要进行转发了，直接共用一根线到单片机就好
*/
void InitCoinInterrupt(void) {
  P0SEL &= ~(0x1 << 4); // P0_4，获取投币信号，设置为0一般功能IO口，通用IO
  P0SEL &= ~(0x1 << 6); // P0_6获取出一个彩票动作
  P0DIR &= ~(0x1<<4);  // 设置为输入功能
  P0DIR &= ~(0x1<<6);
  P0INP &= ~(0x1<<4); // 引脚设置为上拉/下拉模式
  P0INP &= ~(0x1<<6);
  
  P2INP &= ~(0x1<<5); // 所有端口0设置为0上拉
  
  P0IEN |= (0x1 << 4); //  P0_7中断使能
  P0IEN |= (0x1 << 6);
  // PICTL &= ~(0x1 << 0); // 端口0设置为0表示上升沿引起中断
  PICTL |= (0x1 << 0); // 端口0输入的下降沿引起中断
  IEN1 &= ~(0x1 << 5); // 关闭端口0中断，当加入协调器网络的时候再开启中断
  //IEN1 |= (0x1 << 5); // 端口0中断使能
  P0IFG = 0; // 初始化中断标志位  
  EA = 1; // 打开总中断
}
